...
static int __init hook_init(void) {
    ...
    sys_call_table_page = _find_page((unsigned long)p_sys_call_table);

    // Temporarily disable write protection
    *sys_call_table_page = pte_mkwrite(*sys_call_table_page);
    // Overwrite the syscall table entry
    p_sys_call_table[sys_call_num] = my_sys_clone;
    // Re-enable write protection
    *sys_call_table_page = pte_wrprotect(*sys_call_table_page);
    return 0;
}
